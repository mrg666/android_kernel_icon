...
int FSA9485_CheckAndHookAudioDock(int value, int adc)
{
	...
	if (adc == 0x1f) {
		dev_info(&client->dev,
		"%s Off, 0x40, adc == 0x1f, detach AV dock\n", __func__);
	...
	if (value) {
		pr_info("FSA9485_CheckAndHookAudioDock ON\n");
			...
			if (pdata->system_rev() >= 2) {
#endif
				printk(KERN_ERR "%s: path switch -> dock\n",
								__func__);
				ret = i2c_smbus_write_byte_data(client,
...
static void fsa9485_reg_init(struct fsa9485_usbsw *usbsw)
{
	...
	ret = i2c_smbus_write_word_data(client, FSA9485_REG_INT1_MASK, 0x181c);
...
int fsa9485_vbus_valid(void)
{

	struct i2c_client *client = local_usbsw->client;
	int value, ret;

	value = i2c_smbus_read_byte_data(client, FSA9485_REG_RESERVED_1D);
	if (value < 0)
		dev_err(&client->dev, "%s: err %d\n", __func__, value);
...
EXPORT_SYMBOL(fsa9485_vbus_valid);

static int fsa9485_detect_dev(struct fsa9485_usbsw *usbsw)
{
	int device_type, ret, dock_ret = 0, adc = 0;
	...
	if (val1 || val2) {
		...
		/* USB CHARGER */
			...
			dev_info(&client->dev, "USB CHARGER (CDP) connect\n");
		...
		/* Desk Dock */
			...
			dock_ret = FSA9485_CheckAndHookAudioDock(1, adc);
...
*/
	...
	/* Detached */
		...
		/* Desk Dock */
			...
			FSA9485_CheckAndHookAudioDock(0, adc);
...
static irqreturn_t fsa9485_irq_thread(int irq, void *data)
{
	...
	if (intr & 0x20) {
		dev_err(&client->dev, "%s: intr : %02x OVP en!\n",
			__func__, intr);
		msm_batt_is_ovp(1);
	} else {
		if (intr & 0x80) {
			dev_err(&client->dev, "%s: intr : %02x OVP disabled!\n",
				__func__, intr);
			msm_batt_is_ovp(0);
		}
	}
